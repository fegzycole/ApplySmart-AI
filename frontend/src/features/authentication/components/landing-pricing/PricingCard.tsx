...
export function PricingCard({
    ...
    return (
      <Card
        className={cn(
          "relative border-0",
          gradient.card,
          "shadow-2xl shadow-violet-500/50 dark:shadow-violet-900/50 transform scale-105 hover:scale-110 transition-all duration-300",
        )}
      >
  ...
  return (
    <Card
      className={cn(
        "border-2",
        gradient?.card || "border-violet-200/50 dark:border-violet-800/50 bg-white/80 dark:bg-zinc-900/80",
    ...
    >
      <CardHeader>
        ...
        <div className="mt-6">
          <span
            className={cn(
              "text-5xl font-bold",
              gradient?.price || "bg-gradient-to-r from-violet-600 to-fuchsia-600 bg-clip-text text-transparent",
            )}
          >
            {price}
          </span>
      ...
      <CardContent>
            ...
            <li key={feature} className="flex items-start gap-3">
              <div
                className={cn(
                  ...
                  gradient?.check || "bg-violet-100 dark:bg-violet-950/50",
                )}
              >
                <Check
                  className={cn(
                    "size-4",
                    gradient?.check ? gradient.check.replace("bg-", "text-") : "text-violet-600 dark:text-violet-400",
                  )}
                />
              </div>
        ...
        <Button
          ...
          className={cn(
            "w-full",
            buttonVariant === "outline" && "border-2",
            buttonVariant === "outline" && (
              gradient?.card.includes("cyan")
                ? "border-cyan-200 dark:border-cyan-800 hover:bg-cyan-50 dark:hover:bg-cyan-950/30"
                : "border-violet-200 dark:border-violet-800 hover:bg-violet-50 dark:hover:bg-violet-950/30"
            ),
          )}
        >
          {buttonText}
